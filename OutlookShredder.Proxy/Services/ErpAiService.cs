...
public class ErpAiService
{
    ...
    private const string SystemPromptText = """
        ...
        customer_email:     The customer's email address if one is printed on the document (in the contact
                            block, header, or beside the contact name/phone). Return it exactly as shown.
                            Leave null if no email address is printed on the document — do NOT guess one.
    ...
    private static readonly JsonElement _toolJson = JsonDocument.Parse("""
        {
          ...
          "input_schema": {
            ...
            "properties": {
              ...
              "customer_email":     { "type": ["string","null"],   "description": "The customer's email address if printed on the document (contact block/header). Null if no email is printed — do not guess." },
